...
func NewSkippedCreationSameStep(plan *Plan, reg RegisterResourceEvent, new *resource.State) Step {
	contract.Assert(new != nil)
	contract.Assert(new.URN != "")
	contract.Assert(new.ID == "")
	contract.Assert(!new.Custom || new.Provider != "" || providers.IsProviderType(new.Type))
...
func (s *SameStep) IsSkippedCreation() bool {
	return s.skippedCreation
}
